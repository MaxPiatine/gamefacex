...
//Automatically fill in currentslide(n)
cont = [];

for (i = 0; i < imageBox.length; i++) {
    cont.push(imageBox[i].innerHTML);
}

for (i = 0; i < cont.length; i++) {
    imageBox[i].outerHTML = '<div class="imageBox col-lg-4 col-md-6 col-sm-12" onclick="openModal();currentSlide(' + (i + 1) + ')">' + cont[i] + '</div>'
}
